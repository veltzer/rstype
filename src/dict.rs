use crate::utils::{TextLength, dirs_home};
use std::fs;
...
pub fn load_dict_words(lang: &str) -> Vec<String> {
    ...
    content
        ...
        .filter(|w| w.len() >= 2 && w.len() <= 10 && w.chars().all(|c| c.is_ascii_lowercase()))
        .collect()
}
...
pub fn cmd_dict_list_remote() {
    ...
    let resp = ureq::get(url).set("User-Agent", "rstype/1.0").call();
...
pub fn cmd_dict_install(lang: &str) {
    ...
    let resp = ureq::get(&dic_url).set("User-Agent", "rstype/1.0").call();
    let Ok(resp) = resp else {
        eprintln!(
            "Error: failed to download .dic file. Check if '{}' exists at https://github.com/wooorm/dictionaries",
            lang_normalized
        );
        return;
    };
    ...
    eprintln!(
        "Installed {} ({} usable words for word salad)",
        lang_normalized,
        words.len()
    );
}
