...
impl App {
    ...
    fn on_key(&mut self, key: KeyEvent) {
        match self.state {
            ...
            AppState::Waiting | AppState::Typing => {
                match key.code {
                    KeyCode::Backspace => {
                        self.typed.pop();
                    }
                    KeyCode::Char(c) => {
                        ...
                        if self.typed.len() == self.target.len() {
                            let elapsed = self.start_time.unwrap().elapsed();
                            let minutes = elapsed.as_secs_f64() / 60.0;
                            // Word count = chars / 5 (standard definition)
                            let words = self.target.len() as f64 / 5.0;
                            self.wpm = words / minutes;
...
fn render(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &App) -> io::Result<()> {
    terminal.draw(|frame| {
        ...
        match app.state {
            AppState::Waiting | AppState::Typing => {
                render_typing(frame, area, app);
            }
            AppState::Done => {
                render_done(frame, area, app);
            }
        }
    })?;
    Ok(())
...
fn render_typing(
    ...
    // Build the styled text spans
    let mut spans: Vec<Span> = Vec::new();
    for (i, &ch) in app.target.iter().enumerate() {
        let span = if i < app.typed.len() {
            if app.typed[i] == ch {
                Span::styled(ch.to_string(), Style::default().fg(Color::Green))
            } else {
    ...
    let title = match app.state {
        AppState::Waiting => " rstype — press any key to start ",
        _ => " rstype — typing… (Backspace to correct) ",
    };

    let text_width = (app.target.len() as u16 + 4).min(area.width);
    let box_rect = centered_rect(text_width, 5, area);

    let paragraph = Paragraph::new(vec![Line::from(""), text_line])
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(title)
    ...
    // Progress bar
    let progress_pct = app.typed.len() as f64 / app.target.len() as f64;
    let bar_width = box_rect.width.saturating_sub(2) as f64;
    let filled = (bar_width * progress_pct) as usize;
    let empty = bar_width as usize - filled;
    ...
    if bar_rect.bottom() <= area.bottom() {
        frame.render_widget(
            Paragraph::new(bar_text).alignment(Alignment::Center).style(Style::default().fg(Color::Yellow)),
            bar_rect,
        );
    }
}
...
fn render_done(frame: &mut ratatui::Frame, area: Rect, app: &App) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(Span::styled(
            "  Press Enter or R to restart, Ctrl-Q to quit",
            Style::default().fg(Color::DarkGray),
        )),
    ];

    let result_rect = centered_rect(50, 9, area);
    frame.render_widget(
        Paragraph::new(lines).block(
            Block::default()
                .borders(Borders::ALL)
